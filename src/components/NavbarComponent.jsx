  ...
  return (
    <>
      <div className={`h-screen flex-col w-60 flex bg-[#43737e] max-md:w-20`}>
        <img
          className='logo_menu block justify-center content-center select-none p-2 pb-5 ml-5 mt-2 max-w-32 md:max-w-44 max-md:w-16 max-md:ml-2 max-md:mt-6 max-md:p-0 '
          src={logoUni}
        ...
        <div className='bg-[#43737e] max-md:p-3'>
          <ul className='navbar_menu flex justify-between flex-col gap-4 ml-2 p-2 w-56 max-md:mt-20 max-md:ml-0 max-md:absolute'>
            {props.menuItems.map((item, index) => {
              ...
              const isLastItem = index === props.menuItems.length - 1 && item.name === 'Ayuda'; // Verifica si es el último elemento
              const liClass = isLastItem ? 'md:mt-30' : '';
